...
<p>Note: Globally installed packages do not have a <code>package.json</code> semver range available, so their <code>wanted</code> version is <code>latest</code>.</p>
...
unhoisted.</p>
<p>We recommend that package authors use <code>--install-strategy=linked</code> during
...
<code>import</code> of a package that was never added to <code>package.json</code> can fail
instead of resolving by accident and shipping broken. See <a href="../using-npm/developers#catching-undeclared-phantom-dependencies.html">Catching
undeclared ("phantom")
dependencies</a>.</p>
...
<p>As with <code>min-release-age</code>, when this cutoff blocks a fix that <code>npm audit fix</code> would install, npm keeps the vulnerable version, warns, and exits with
a non-zero code.</p>
<p>Packages whose names match <code>min-release-age-exclude</code> are exempt from this
filter.</p>
...
<p>When this window stops <code>npm audit fix</code> from installing a patched version
...
<code>min-release-age-exclude</code>, or relax <code>min-release-age</code> or <code>before</code>.</p>
<p>Packages whose names match <code>min-release-age-exclude</code> are exempt from this
filter.</p>
<p>This value is not exported to the environment for child processes.</p>
<h4 id="min-release-age-exclude"><code>min-release-age-exclude</code></h4>
<ul>
<li>Default:</li>
<li>Type: String (can be set multiple times)</li>
</ul>
<p>A list of package names or <code>minimatch</code> glob patterns that are exempt from
the <code>min-release-age</code> (and <code>before</code>) filter. A matching package can always
resolve to its newest version, even when a release-age window is set.</p>
<p>For example, to apply a release-age window to third-party dependencies while
letting internally maintained packages update immediately:</p>
<pre><code>min-release-age=7
min-release-age-exclude[]=@myorg/*
min-release-age-exclude[]=my-internal-pkg
</code></pre>
<p>Only the named package is exempt; its own dependencies still follow the
release-age policy unless they also match a pattern. Patterns match against
the package name, so <code>@myorg/*</code> matches <code>@myorg/shared-utils</code>.</p>
<p>Excluding a package does not change which registry it is fetched from. You
should own your private scope on the public registry so that nobody else can
publish a package with the same name.</p>
<p>This value is not exported to the environment for child processes.</p>
