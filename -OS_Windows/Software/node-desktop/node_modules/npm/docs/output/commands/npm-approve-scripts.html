...
denied.</p>
<p>If a registry dependency has no <code>resolved</code> URL in your <code>package-lock.json</code>
(for example, an older lockfile or one written with
<code>omit-lockfile-registry-resolved</code>), npm cannot verify a trusted version for
it and cannot pin it: a <code>pkg@1.2.3</code> entry never matches, so the package
keeps appearing under <code>--allow-scripts-pending</code>. <code>approve-scripts</code> approves
these by name (<code>pkg: true</code>) and warns when it does. To restore pinning,
refresh the lockfile with <code>npm install</code>.</p>
